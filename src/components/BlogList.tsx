              ...
              {filtered.map((p) => (
                <Link key={p.slug} href={`/blog/${p.slug}`} className="card-hover"
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
                    <span style={{ fontSize: 12, padding: "4px 12px", borderRadius: 100, background: catColors[p.category] ?? "rgba(91,124,250,0.1)", color: catText[p.category] ?? "var(--primary)", fontWeight: 500 }}>{p.category}</span>
                    {p.videoUrl && <span style={{ fontSize: 11, color: "#34d399" }}>▶ Video</span>}
                  </div>
