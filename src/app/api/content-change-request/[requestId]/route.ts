    ...
    emailJob = () => sendMail({
        ...
        html: `
          <div style="font-family:system-ui,sans-serif;max-width:520px;padding:28px">
            <h2>Hi ${escHtml(owner.name)},</h2>
            <p>A user has suggested an edit on your content <strong>${escHtml(cr.contentTitle)}</strong>, and the admin has reviewed and forwarded it to you.</p>
            <div style="background:#f3f4ff;border:1px solid #dde;border-radius:10px;padding:16px 20px;margin:20px 0">
              <div style="font-size:12px;color:#666;margin-bottom:6px">Suggestion from ${escHtml(cr.requestorName)}</div>
              <div style="font-size:14px;color:#1a1a2e;line-height:1.6">${escHtml(cr.suggestion)}</div>
            </div>
    ...
    emailJob = () => sendMail({
      ...
      html: `
        <div style="font-family:system-ui,sans-serif;max-width:520px;background:#0a0e1a;color:#eef2ff;padding:28px;border-radius:12px">
          ...
          <p style="color:#9ba8c4;font-size:14px">The content owner has confirmed they will make the requested edit on <strong style="color:#eef2ff">${escHtml(cr.contentTitle)}</strong>.</p>
          ${note ? `<p style="font-size:13px;color:#9ba8c4">Owner note: ${escHtml(note)}</p>` : ""}
      ...
      await sendMail({
        ...
        html: `
          <div style="font-family:system-ui,sans-serif;max-width:520px;margin:0 auto;padding:32px">
            <h2 style="margin-bottom:8px">Hi ${escHtml(cr.requestorName)},</h2>
            <p style="color:#555;line-height:1.7">
              Your suggestion on <strong>${escHtml(cr.contentTitle)}</strong> has been reviewed, accepted, and the content has been updated. Thank you for helping make Qurious Academy better!
            </p>
            <div style="margin:28px 0;padding:20px 24px;background:#f3f4ff;border:2px dashed #5b7cfa;border-radius:10px;text-align:center">
              ...
              <div style="font-size:32px;font-weight:800;letter-spacing:0.15em;color:#1a1a2e">${escHtml(couponCode)}</div>
