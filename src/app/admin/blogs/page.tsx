...
type Post = { id: string; slug: string; title: string; category: string; author: string; published: boolean; createdAt: string };
...
export default function AdminBlogsPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/blogs").then(r => r.json()).then(d => { setPosts(d); setLoading(false); });
  }, []);
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {posts.map(p => (
            <div key={p.id} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 18px", display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <Link href={`/admin/blogs/${p.id}`} style={{ fontSize: 12, color: "var(--primary)", textDecoration: "none", flexShrink: 0 }}>Edit</Link>
              <button onClick={() => deletePost(p.id)} style={{ fontSize: 12, color: "#ef4444", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}>
                Delete
              </button>
            </div>
          ))}
